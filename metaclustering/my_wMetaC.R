wMetaC_with_confidence <- function(nC, hmethod = hmethod, enN.cluster = enN.cluster, minN.cluster = minN.cluster, maxN.cluster = maxN.cluster, sil.thre = sil.thre, height.Ntimes, confidence_ratio = confidence_ratio) {
  ...
  file_path <- "./scMCTL-Pipeline/metaclustering/output_data/Voting_cluster.csv"
  ...
  pca_data <- read.csv("./scMCTL-Pipeline/clustering/output_data/multi_clusts_combined.csv", encoding='utf-8')
  adjusted_data <- cbind(newnC, cell_name = pca_data[, 1])
  write.csv(adjusted_data, "./scMCTL-Pipeline/metaclustering/output_data/adjusted_Voting_cluster_with_cells.csv", row.names = FALSE)
  
  adjusted_file_path <- "./scMCTL-Pipeline/metaclustering/output_data/adjusted_Voting_cluster_with_cells.csv"
  ...
  TOP_percent_path <- "./scMCTL-Pipeline/metaclustering/output_data/TopConfidence_cells.csv"
