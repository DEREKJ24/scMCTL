...
def step_2_pca_kmeans(pca_file_path, dimensions_list, max_k, clustering_script):
    ...
    os.makedirs('./scMCTL-Pipeline/clustering/output_data', exist_ok=True)
...
if __name__ == "__main__":
    ...
    raw_data_path = input("Please enter the path of the original gene expression matrix (for example: E:/scMCTL_Pipeline/processing/input_data/GSE247600.csv):")
    ...
    # Step 1
    preprocessing_script = './scMCTL-Pipeline/preprocessing/preprocessing.R'
    output_processed_path = './scMCTL-Pipeline/preprocessing/output_data/gene_expression_afterprocess.csv'
    output_variable_genes_path = './scMCTL-Pipeline/preprocessing/output_data/gene_expression_2000genes.csv'

    # Step 2
    pca_file_path = output_variable_genes_path
    dimensions_list = [30, 35, 40, 45, 50]
    max_k = 20
    clustering_script = './scMCTL-Pipeline/clustering/PCA_multiK_cluster.py'

    # Step 3
    input_clusters_path = './scMCTL-Pipeline/clustering/output_data/multi_clusts_combined.csv'
    metaclustering_script = './scMCTL-Pipeline/metaclustering/Main_wMetaC.R'
    target_cluster = int(input("Please enter the number of target clusters (positive integer, default: 8): ") or 8)

    # Step 4
    output_predictions_path = './scMCTL-Pipeline/transformer/output_data/predicted_clusters_full.csv'
    transformer_script = './scMCTL-Pipeline/transformer/transformer4.py'
